...
export default class ProductForm {
  categoriesData = [];
  imageContainer = null;  //указатель на контейнер для списка изображений
  ...
  async render () {
    ...
    this.formData.images.forEach(item => {
      ...
      buffer.innerHTML = this.renderImageListItem(item);
      imageElements.push(buffer.firstElementChild);
    });

    this.imageContainer = new SortableList({items: imageElements});

    this.subElements.imageListContainer.append(this.imageContainer.element);
  ...
  save = async () => {
    ...
    const imageElements = this.imageContainer.element.querySelectorAll('.sortable-table__cell-img');
  ...
  uploadImage = () => {
    ...
    fileInput.addEventListener('change', async () => {
      ...
      if (file) {
        ...
        this.imageContainer.addItem(imageElement.firstElementChild);
